export default function SystemHealthHistory({ history = [] }) {
  ...
  return (
    <div className="rounded-lg border border-border-default bg-bg-surface p-6 shadow-soft h-full flex flex-col justify-between">
      <div>
          ...
          {history.map((row, index) => (
            <div key={index} className="flex items-center gap-3">
              ...
              <span className={`w-12 text-right text-xs font-bold ${
                parseFloat(row.uptime) < 95 ? "text-[#c62828]" : "text-[#006c49]"
              }`}>
                {row.uptime}
              </span>
            </div>
